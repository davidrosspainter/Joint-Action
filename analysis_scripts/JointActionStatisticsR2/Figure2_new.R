...
single_session_directory = set_output_directory("Figure2_new/single_session/")


is_load_fresh = TRUE
is_plot_group_endpoints = FALSE
is_plot_group_trajectories = TRUE
...
endpoint_plot_pdf <- function(df, plot_title, filename){
  ...
  p <- force(ggplot(df) +
               ...
               theme(
                 ...
                 axis.title = element_text(size = 11*scale_modifer),
                 axis.text = element_text(size = 11*scale_modifer),
                 
                 axis.line.y = element_line(colour = 'black', size = 1*(10/11.875)*scale_modifer), # .0353/.0419
                 axis.line.x = element_line(colour = 'black', size = 1*(10/11.875)*scale_modifer), # .0353/.0419
                 axis.title.x = element_text(margin = margin(t = 0, r = 0, b = 0, l = 0)),
                 axis.title.y = element_text(margin = margin(t = 0, r = 0, b = 0, l = 0)),
                 
                 plot.title = element_text(
                   size = 14*scale_modifer,
                   hjust = .5,
                   face = 'plain',
                   margin = margin(t = 5, r = 0, b = 0, l = 0)
                 ),
                 axis.line = element_line(colour = 'black', size = 1*(10/11.875)*scale_modifer),
                 axis.ticks = element_line(colour = "black", size = 1*(10/11.875)*scale_modifer)
               ) +
               coord_fixed()
             )
  ...
  for (TARGET in seq(1, number_of_targets)) {
    p <- p + annotate( "path",
                       x = array.x[TARGET] * dpp + sizes.target / 2 * cos(seq(0, 2 * pi, length.out = 100)),
                       y = array.y[TARGET] * dpp + sizes.target / 2 * sin(seq(0, 2 * pi, length.out = 100)),
                       size = scale_modifer/2)
  }
...
endpoint_plot_by_condition <- function(){
  ...
  for (i in seq(1, nrow(figure_key))){
    ...
    if (CONTROL %in% str_control[1:2]){
      ...
      filename <- paste0(single_session_directory, 'endpoint.', paste(as.character(SESSION), str_figure[FIGURE], str_control[PLAYER], VISIBILITY), '.png')
      ...
      plot_title <- paste(str_control[PLAYER], VISIBILITY)
      filename <- paste0(single_session_directory, 'trajectory.', paste(as.character(SESSION), str_figure[FIGURE], str_control[PLAYER], VISIBILITY), '.png')
      
      df.x <- as_tibble(as.data.frame.table(drop(trajectory[1,,,PLAYER])))
      names(df.x) <- c('frame', 'trial', 'x')
      
      df.y <- as_tibble(as.data.frame.table(drop(trajectory[2,,,PLAYER])))
...
if (is_load_fresh){
  ...
  save(DF, DF2, ED, file = paste0(output_directory, "DF.RData"))
  
} else{
  
  load(file = paste0(output_directory, "DF.RData"))
...
if(is_plot_group_endpoints){

  # figure_key = figure_key[1,]
  ...
  group_plot <- function(DF, filename_modifier){
    ...
    for (i in seq(1, nrow(figure_key))){
      ...
      filename <- paste0(output_directory, 'group.', filename_modifier, '.', paste(figure_key[i,]$CONTROL, figure_key[i,]$VISIBILITY), '.png')
      filename2 <- paste0(output_directory, 'group.', filename_modifier, '.', paste(figure_key[i,]$CONTROL, figure_key[i,]$VISIBILITY), '.pdf')
      
      if(filename_modifier == "endpoint"){
        #subplots[[i]] <- endpoint_plot(df[[i]], plot_title, filename)
        endpoint_plot_pdf(df[[i]], plot_title, filename2)
      }
      else {
        #subplots[[i]] <- trajectory_plot(df[[i]], plot_title, filename)
      }
    }
...
# trajectory group plot ---------------------------------------------------

if (is_plot_group_trajectories){

  #figure_key = figure_key[1,]
  
  graphics.off()
  
  trajectory_plot_base1 <- function(df, plot_title){
  ...
  trajectory_plot_base2 <- function(p, df){
    p <- p + geom_path(aes(x=x, y=y, group=trial),
                           data=df,
                           color = col_target[df$target_position],
                           size = .25,
                           alpha = .5)
  ...
  for (i in seq(1, nrow(figure_key))){
    ...
    filename_modifier <- 'trajectory'
    plot_title <- paste(figure_key[i,]$CONTROL, figure_key[i,]$VISIBILITY)
    filename <- paste0(output_directory, 'group.', filename_modifier, '.', paste(figure_key[i,]$CONTROL, figure_key[i,]$VISIBILITY), '.pdf')
    
    for(SESSION in c(1:globals$number_of_sessions)){
      if (SESSION == 1){ p <- trajectory_plot_base1(DF2[[SESSION]][[i]], plot_title) }
      else{ p <- trajectory_plot_base2(p, DF2[[SESSION]][[i]])}
    }
    
    scale_modifer = 1.5
    
    for (TARGET in seq(1, number_of_targets)) {
      p <- p + annotate( "path", x = array.x[TARGET] * dpp + sizes.target / 2 * cos(seq(0, 2 * pi, length.out = 100)),
                                 y = array.y[TARGET] * dpp + sizes.target / 2 * sin(seq(0, 2 * pi, length.out = 100)),
                                 size = scale_modifer/2)
    }
