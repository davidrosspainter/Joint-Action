...
addpath 'D:\JOINT.ACTION\synchronyResults\synchronyResults\Colormaps\pmkmp\'
addpath 'D:\JOINT.ACTION\JointActionRevision\analysis\external\topoplot_hack\'
addpath 'D:\JOINT.ACTION\JointActionRevision\analysis\external\'

addpath 'D:\JOINT.ACTION\JointActionRevision\analysis\common\'
addpath 'Colormaps\'
...
for CC = 1:n.cond+1
    ...
    if CC < 3
        imagesc( win.t, [], m(:,order2,CC)' )
        caxis(limit)
    else
        dm = m(:,:,2) - m(:,:,1);
        imagesc( win.t, [], dm(:,order2)' )
...
line( get(gca,'xlim'), find(order2==i2)*[1 1], 'color', 'r', 'linewidth', .1 )
...
topoplot( dm(tIDX2,:), chanlocs, 'maplimits', mapLimits, 'electrodes', 'on', 'shading', 'interp', 'conv', 'on')
%title(win.t(tIDX2))
