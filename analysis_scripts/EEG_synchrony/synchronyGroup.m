...
if is_load_fresh
    ...
    %% ----- load epoch data ----- MEMORY INTENSIVE!!!!!
    ...
    for SUBJECT = 1:number_of_subjects
        ...
        [SESSION, PLAYER, FNAME{SUBJECT}] = generate_subject_string(SUBJECT, generate_subject_code);
        tmp = load( [ IN FNAME{SUBJECT}.SUBJECT '.epoch.mat' ] );
...
%% ---- time consuming - 5 minutes/electrode at 8 cores ---- 8 hours!
...
if is_correlate
    ...
    for ELECTRODE = 1:number_of_channels
        ...
        parfor EE = 1:nEpochs  
            RES{EE} = permF(epoch2use, win, EE, type2);
        end
