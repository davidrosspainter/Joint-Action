...
public class Test5 : MonoBehaviour
{
    ...
    float[] yMod = new float[2] { +1080/2, -1080/2 };
    ...
    void Start()
    {
        ...
        foreach (var SESSION in sessionsToUse)
        {
            print("***************************************************************");
            print("SESSION: " + SESSION.ToString());

            GameObject grandParent = new GameObject(name: Labels.session[SESSION]);

            float yModToUse = 0 - ((sessionCount - 1) * 1080);

            for (int CONTROL = 0; CONTROL < Number.control; CONTROL++)
            {
                DrawTargets(xMod[CONTROL], yModToUse, "targets." + Labels.control[CONTROL], grandParent);
            }

            Ports2 ports2 = new Ports2(SESSION * 100);

            float[,] target_positions_solo = GetBufferData(ports2.target_positions_solo);

            AddCursors(ports2.P1, "P1", CursorType.cursor, xMod[0], yModToUse, target_positions_solo, grandParent);
            AddCursors(ports2.P2, "P2", CursorType.cursor, xMod[0], yModToUse, target_positions_solo, grandParent);

            AddCursors(ports2.P1_gaze_solo, "P1_gaze_solo", CursorType.gaze, xMod[0], yModToUse, target_positions_solo, grandParent);
            AddCursors(ports2.P2_gaze_solo, "P2_gaze_solo", CursorType.gaze, xMod[0], yModToUse, target_positions_solo, grandParent);

            float[,] target_positions_joint = GetBufferData(ports2.target_positions_joint);

            AddCursors(ports2.joint, "joint", CursorType.cursor, xMod[1], yModToUse, target_positions_joint, grandParent);

            AddCursors(ports2.P1_gaze_joint, "P1_gaze_joint", CursorType.gaze, xMod[1], yModToUse, target_positions_joint, grandParent);
            AddCursors(ports2.P2_gaze_joint, "P2_gaze_joint", CursorType.gaze, xMod[1], yModToUse, target_positions_joint, grandParent);
    ...
    class Ports2
    {
        public int[] P1 = new int[2] { 1, 2 };
        public int[] P2 = new int[2] { 3, 4 };
        public int target_positions_solo = 5;
        public int[] P1_gaze_solo = new int[2] { 6, 7 };
        public int[] P2_gaze_solo = new int[2] { 8, 9 };

        public int[] joint = new int[2] { 10, 11 };
        public int target_positions_joint = 12;
        public int[] P1_gaze_joint = new int[2] { 13, 14 };
        public int[] P2_gaze_joint = new int[2] { 15, 16 };

        public Ports2(int constant)
        {
            AddConstantToArray(P1, constant);
            AddConstantToArray(P2, constant);
            target_positions_solo += constant;
            AddConstantToArray(P1_gaze_solo, constant);
            AddConstantToArray(P2_gaze_solo, constant);

            AddConstantToArray(joint, constant);
            target_positions_joint += constant;
            AddConstantToArray(P1_gaze_joint, constant);
            AddConstantToArray(P2_gaze_joint, constant);
        }

        void AddConstantToArray(int[] array, int constant)
        {
